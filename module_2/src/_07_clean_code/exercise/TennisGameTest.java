...
public class TennisGameTest {
    ...
    public void checkAllScores() {
        int highestScore = Math.max(this.playerFirstName, this.playerSecondName);
        int mScoreFirst = 0;
        int mScoreSecond = 0;
        for (int i = 0; i < highestScore; i++) {
            if (i < this.playerFirstName)
                mScoreFirst += 1;
            if (i < this.playerSecondName)
                mScoreSecond += 1;
        }
        assertEquals(this.expectedScore, TennisGame.getScore("John", "Bill", mScoreFirst, mScoreSecond));
    }
}
